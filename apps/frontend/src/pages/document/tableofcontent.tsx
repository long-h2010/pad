...
const TableOfContent: React.FC<any> = (props) => {
    ...
    useEffect(() => {
        (refContent.current as any) = editorRef.current;
    }, [editorRef.current]);
            ...
            if (refContent.current && refContent.current.contentDocument) {
                ...
                if (contentDocument) {
                    const elements = Array.from(
                        contentDocument.querySelectorAll<HTMLElement>('h2, h3, h4')
                    ).map((elem, index) => ({
    ...
    return (
        <nav>
            {headings.length > 0 && (
                <h5 style={{ color: 'black', fontWeight: 'bold' }}>Table of contents</h5>
            )}
            <ul>
